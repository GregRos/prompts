...
export const textTillCloserWithAlt = <T extends string>(closer: T) => {
    const altSection = string("|").pipe(
        then(anyCharOrEscape.pipe(manyTill(closer)))
    )
    const tillAltSectionOrCloser = manyTill(
        altSection.pipe(or(closer)),
        (_main, till) => {
...
export const anyCharOrEscape = anyChar().pipe(
    or(string("\\").pipe(qthen(anyChar())))
)
