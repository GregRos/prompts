...
const argDest = "C:\\Users\\Greg\\AppData\\Roaming\\Code - Insiders\\User\\prompts"
...
async function syncOnce(src: string, dest: string) {
    ...
    for (const rel of relFiles) {
        ...
        const folder =
            parent === "." || parent === "" ? posix.basename(src) : parent.split("/").pop()
    ...
    for (const rel of destFiles) {
        ...
        if (!expectedDestNames.has(base)) {
            ...
            console.log(`[remove] extra ${rel} from ${path.relative(process.cwd(), dest)}`)
        }
    }
