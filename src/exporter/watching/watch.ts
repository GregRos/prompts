...
export type ChangeEventOf<T extends EventName> = Extract<_ChangeEvent, { type: T }>
...
function getNormalEvents$(watcher: FSWatcher) {
    const normalEvents = ["add", "change", "unlink", "addDir", "unlinkDir"] as const
    return merge(
        ...normalEvents.map(event =>
            getEventArgs$(watcher, event, (path: string, stats: Stats | undefined) => ({
                type: event,
                path,
                stats
            }))
...
export function watchDir(targetDir: string): Observable<_ChangeEvent> {
    return defer(() => {
        ...
        const error$ = fromWatcherEvent<[error: Error], _ChangeEvent>(watcher, "error", error => {
            throw error
        })
